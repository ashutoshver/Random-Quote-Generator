...
function printQuote() {
  ...
  var quoteString = `<p id="quote">${quotes.quote}</p><p id="source">${quotes.source}`;
  if (quotes.citation) { quoteString += `<span id="citation">${quotes.citation}</span>` }
  if (quotes.year) { quoteString += `<span id="year">${quotes.year}</span></p>` }
  else { quoteString += `</p>` };
